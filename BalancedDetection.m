...
yi = [218, 233, 248, 268];          % detector row (px) of each reference line
xi = [436.6, 487.7, 546.5, 611.6];  % corresponding wavelength (nm)
x0 = 91;                            % horizontal offset (px) of the 0th order
...
addpath('D:\HongjingMao\bfmatlab')

% Input ND2 stack.
nd2FilePath = 'D:\Miami\IN-BOP_B4\589\002.nd2';
...
outputFilePath = 'Output_path\1.xlsx';   % full Excel path; folder is derived from it
...
A = rot90(A, 2);   % orientation correction (180 deg, per acquisition convention)
A9 = A;
...
% is below threshold (107) are treated as signal-free and averaged to form
% the per-row background profile.
back = [];
for n = 1:size(A, 1)
    w = squeeze(A(n, 150:350, 1:end));
    w = mean(w, 1);
    a = w < 107;                       % signal-free frame mask for this row
    back(n, :) = mean(A(n, :, a), 3);
end
clf
imagesc(back)
...
for n = 1:size(A, 3)
    b = A(:, slit-3:slit+3, n);
    s = sum(b, 2);
    t = 20;                            % minimum peak height (photon threshold)
    warning off
...
convTol      = 0.005;   % nm; stop early once consecutive rounds agree within this tolerance
...
% Per-event photon budget (calibrated using the 600-800 nm range).
s = bb3*1.1/0.95;

% Filter events by photon budget.
photonMin = 750;        % lower photon bound
photonMax = 3500;       % upper photon bound
a1 = s >= photonMin;
a2 = s <= photonMax;
a  = logical(a1 .* a2);
s   = s(a);
...
imagesc(spe_plot(1:nShow, :), [10 500])
colormap(gca, hot)
